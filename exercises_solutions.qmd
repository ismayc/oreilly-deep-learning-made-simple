...
opt = torch.optim.Adam(net.classifier.parameters(), lr=1e-3)  # only the head has gradients
loss_fn = nn.CrossEntropyLoss()       # expects raw logits + integer labels

# Reuse the same images/labels; torchvision wants channels-first tensors in [0, 1]
X_img_train_t = torch.tensor(X_img_train).permute(0, 3, 1, 2) / 255.0  # NHWC -> NCHW
...
train_loader = DataLoader(
    TensorDataset(X_img_train_t, y_img_train_t), batch_size=16, shuffle=True)
...
for epoch in range(5):
    ...
    for x_batch, y_batch in train_loader:
        ...
        loss = loss_fn(net(prep(x_batch)), y_batch)  # prep applies the model's normalization
        loss.backward()                      # measure how to improve
        opt.step()                           # nudge the head's weights
...
for j in range(len(features)):
    ...
    X_shuffled[:, j] = rng.permutation(X_shuffled[:, j])   # break this one feature's signal
    acc = ((model.predict(X_shuffled, verbose=0).ravel() > 0.5).astype(int) == y_val).mean()
    drops.append(base_acc - acc)

order = np.argsort(drops)
plt.barh([features[i] for i in order], [drops[i] for i in order], color="steelblue")
plt.xlabel("accuracy lost when the feature is shuffled")
plt.title("What drives the cancellation model?"); plt.tight_layout(); plt.show();
```
