...
author:
  - name: "Dr. Chester Ismay"
    url: "https://chester.rbind.io"
format:
  html:
    theme: flatly
    toc: true
...
```{=html}
<style>
.cell-output-display:has(.mermaid-js) { text-align: center; }
.cell-output-display .mermaid-js { display: inline-block; max-width: 100%; }
.cell-output-display .mermaid-js > svg { max-width: 100%; height: auto; }
.cell-output-display .mermaid-js g.eng rect, .cell-output-display .mermaid-js g.engUsed rect { width: 130px !important; x: -65px !important; }
</style>
```

```{python}
#| eval: false
# Run this once per session, then RESTART the runtime if
# Colab prompts you to. Colab already ships torch; we
# add/upgrade Keras 3 and the datasets library.
!pip install -q -U keras datasets
!pip install -q scikit-learn matplotlib pillow
```

```{python}
# Run this setup cell FIRST. The backend must be chosen
# BEFORE keras is imported.
import os
...
print("Keras", keras.__version__, "on backend:",
...
computation underneath. Every walkthrough also includes an **idiomatic PyTorch**
...
```{mermaid}
%%| fig-cap: "You write Keras 3 once; one setting picks the engine that runs it (PyTorch here)."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}, 'flowchart': {'nodeSpacing': 90}}}%%
flowchart TD
  C(["Your Keras 3 code"]):::proc --> E{"KERAS_BACKEND ?"}:::decision
  E -->|tensorflow| F["TensorFlow"]:::eng
  E -->|torch| T["PyTorch"]:::engUsed
  E -->|jax| J["JAX"]:::eng
  F --> R(["Same model,<br/>runs unchanged"]):::good
  T --> R
  J --> R
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef decision fill:#fff3cd,stroke:#d39e00,color:#1a1a1a
  classDef eng fill:#e6e0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef engUsed fill:#cdb8ff,stroke:#5b1690,stroke-width:3px,color:#1a1a1a
...
|---|---|---|
| **PyTorch** *(our choice)* | Pythonic, easy to debug; huge model and tutorial ecosystem; now common in industry | Serving tooling historically less turnkey than TensorFlow |
| **TensorFlow** | Mature deployment (TF Serving, TFLite, on-device); proven at scale | More boilerplate; less handy for quick, interactive work |
| **JAX** | Very fast on TPUs; composable `jit`, `grad`, `vmap` for research | Smaller ecosystem; steeper curve; least beginner-friendly |
...
```{python}
#| eval: false
# To run this course on the TensorFlow engine instead of
# PyTorch, set this BEFORE `import keras`, then restart the
# runtime (the engine is locked in once Keras imports).
import os
...
## Walkthrough 0.1: Setting Up Keras (PyTorch backend) and Your First Tensor
...
```{python}
# A tensor is just an array the framework can run fast math
# on (and learn from)
...
## Exercise 0.1: Confirm Your Environment

```{python}
# Build a tiny one-line "model" and run data through it:
...
```{mermaid}
%%| fig-cap: "Fit the scaler on training data only, then apply it to both splits. Computing it on all the data leaks the future."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  TR[("Training set")]:::data --> SC["Compute mean & std<br/>on TRAIN only"]:::key
  SC --> AP["Apply the same numbers<br/>to BOTH splits"]:::proc
  AP --> TS[/"Scaled training"/]:::good
  AP --> VS[/"Scaled validation"/]:::good
...
```{python}
# A handful of numeric drivers a revenue manager would
# recognize
...
# Fit the shift/scale on the TRAINING data only, then apply
# the SAME numbers to validation (see the pitfall below).
...
```{mermaid}
%%| fig-cap: "Business inputs flow through hidden 'specialist' layers into one cancellation probability."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart LR
  I[/"8 booking<br/>features"/]:::data --> H1["Dense 32<br/>specialists"]:::proc
  H1 --> H2["Dense 16<br/>combine"]:::proc
  H2 --> O[/"Cancel<br/>probability"/]:::good
  classDef data fill:#d1f0ec,stroke:#0b7a75,color:#1a1a1a
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
```
...
```{mermaid}
%%| fig-cap: "Training is a loop: predict, measure how wrong, adjust, and repeat."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '17px'}}}%%
flowchart LR
  P["Predict"]:::proc --> M["Measure error<br/>= loss"]:::bad
  M --> A["Adjust weights<br/>= optimizer"]:::accent
  A -->|repeat each epoch| P
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
...
```{python}
# A model that "separates" the classes pushes cancellations
# toward 1 and the rest toward 0. Overlapping histograms =
# an indecisive model.
val_probs = model.predict(X_val, verbose=0).ravel()
plt.hist(val_probs[y_val == 0], bins=30, alpha=0.6,
         label="actually kept")
plt.hist(val_probs[y_val == 1], bins=30, alpha=0.6,
         label="actually canceled")
plt.axvline(0.5, color="k", ls="--", lw=1,
            label="default threshold")
plt.xlabel("predicted cancellation probability")
plt.ylabel("number of bookings")
plt.legend()
plt.title("Separation is the model's confidence, made visible")
plt.show();
```
...
# Convert the same train/val arrays into PyTorch tensors
# (the "_t" = tensor version).
X_train_t = torch.tensor(X_train)
# shape (N,) -> (N, 1) to match the model output
y_train_t = torch.tensor(y_train).unsqueeze(1)
X_val_t = torch.tensor(X_val)
y_val_t = torch.tensor(y_val).unsqueeze(1)

# Idiomatic PyTorch feeds the model mini-batches via a
# DataLoader
train_loader = DataLoader(
    TensorDataset(X_train_t, y_train_t), batch_size=256,
    shuffle=True)

net = nn.Sequential(
    nn.Linear(len(features), 32), nn.ReLU(),
    nn.Linear(32, 16), nn.ReLU(),
    # outputs a raw score ("logit"), not a probability
    nn.Linear(16, 1),
)
optimizer = torch.optim.Adam(net.parameters(), lr=1e-3)
# applies the sigmoid for us, more numerically stable
loss_fn = nn.BCEWithLogitsLoss()

# Train: loop over the data 10 times, one gradient step per
# mini-batch
...
with torch.no_grad():
    ...
    # 1 where the prediction matches the truth
    correct = (predicted == y_val_t).float()
    # fraction correct
...
> **Prefer TensorFlow?** The same model is identical save for the prefix, e.g.
> `tf.keras.Sequential([...])`, since Keras 3 *is* the API TensorFlow exposes. Like the
> Keras code above it hands you `.fit()`; only the lower-level PyTorch version needs a
> hand-written loop.
...
```{mermaid}
%%| fig-cap: "Switch tasks by changing only the last layer and the loss; the body stays the same."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  B["Same network body"]:::proc --> Q{"Predicting<br/>what?"}:::decision
  Q -->|Yes / No| C["Sigmoid + cross-entropy<br/>probability, 0 to 1"]:::good
  Q -->|A number| R["No activation + MSE<br/>any dollar value"]:::accent
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef decision fill:#fff3cd,stroke:#d39e00,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
...
# Keep rows with no missing features, a valid price, and a
# sensible range (drop outliers > $1000)
keep = (
    reg_features.notna().all(axis=1)
    & adr_price.notna()
    & (hotels["adr"].between(0, 1000))
)
...
# Standardize on the training split only, then apply to
# validation (same rule as before)
...
```{python}
# STEP 1: Build the model. Two choices flip it from a
# classifier to a REGRESSION model:
#   * the output Dense layer has NO activation, so it can
#     predict any dollar amount (not squashed to 0-1)
#   * compile with loss "mse" and metrics ["mae"], so the
#     score reads as dollars off
reg_model = keras.Sequential([
    keras.layers.Input(shape=(X_reg.shape[1],)),
    keras.layers.Dense(32, activation="relu"),
    keras.layers.Dense(16, activation="relu"),
    # no activation -> predict any number
...
```{python}
# STEP 2: Train and report the validation MAE (dollars off,
# on average)
reg_hist = reg_model.fit(
    X_reg_train, y_reg_train,
    validation_data=(X_reg_val, y_reg_val), epochs=10,
...
```{python}
# Predicted vs actual price: points on the dashed line are
# perfect predictions
...
plt.legend()
plt.title("Where the price model is tight, and where it drifts")
plt.show();
```
...
```{python}
# Scratch space for 1.2: paste an error or a model summary
# here and work with your AI assistant. (Nothing to run yet;
# this cell is yours to experiment in.)
```
...
1. The classification model reaches ~75-76% accuracy. With ~63% of bookings *not*
...
for row in food:
    ...
    if len(samples) >= 6:
        break
print("Example categories in this dataset:",
      class_names[:8], "...")
```
...
```{mermaid}
%%| fig-cap: "A CNN builds understanding in stages, from tiny edges up to the whole object."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  P[/"Raw pixels"/]:::s1 --> E["Edges & corners"]:::s2
  E --> T["Textures & shapes"]:::s3
  T --> PA["Object parts"]:::s4
  PA --> L[/"'coffee maker'"/]:::good
  classDef s1 fill:#eef4ff,stroke:#1a73e8,color:#1a1a1a
  classDef s2 fill:#cfe0ff,stroke:#1a73e8,color:#1a1a1a
  classDef s3 fill:#a9c7ff,stroke:#1a4fa0,color:#1a1a1a
  classDef s4 fill:#7fa9f5,stroke:#1a4fa0,color:#10203a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
```
...
def predict_label(pil_img):
    ...
    # [None] wraps one image in a batch of 1
    batch = np.array(img, dtype="float32")[None]
    # the preprocessing this model expects
    arr = preprocess_input(batch)
    top = decode_predictions(
        pretrained.predict(arr, verbose=0), top=1)[0][0]
    # (label, confidence)
    return top[1], round(float(top[2]), 2)
...
```{python}
# Show the images with the model's guess: seeing the
# picture makes the result land
import matplotlib.pyplot as plt
...
```{python}
# Build a small labeled set from 3 food categories (your
# "product catalog")
wanted = ["pizza", "hamburger", "sushi"]
# Map each Food-101 category index to a fresh 0/1/2 label
# for our 3-class problem
want_idx = {class_names.index(name): new_label for new_label, name in enumerate(wanted)}

imgs, labels = [], []
# how many we've collected per category
counts = {category: 0 for category in want_idx}
for row in load_dataset("ethz/food101", split="train",
                        streaming=True):
    category = row["label"]
    # cap at 120 images each
    if category in want_idx and counts[category] < 120:
...
X_img_train, X_img_val, y_img_train, y_img_val = train_test_split(
    X_img, y_img, test_size=0.25, random_state=2026,
    stratify=y_img)
print("Training images:", X_img_train.shape, "across",
      len(wanted), "categories")
```

```{mermaid}
%%| fig-cap: "Keep the giant model's general vision frozen; train only a small new head on your own data."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart LR
  V["Pre-trained base (FROZEN)<br/>vision from<br/>millions of images"]:::frozen --> NH["New head you train<br/>on YOUR 270 training photos"]:::accent
  NH --> OUT[/"pizza /<br/>burger /<br/>sushi"/]:::good
  classDef frozen fill:#dbe9ff,stroke:#1a73e8,stroke-width:3px,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
```

```{python}
# Freeze the pre-trained base; train only a new
# classification head
...
clf.fit(X_img_train, y_img_train,
        validation_data=(X_img_val, y_img_val), epochs=5,
        batch_size=16, verbose=0)
print(
    "Validation accuracy:",
    round(float(clf.evaluate(X_img_val, y_img_val, verbose=0)[1]), 3))
```

```{python}
# Show 6 validation photos with the fine-tuned model's call
# (green = right, red = wrong)
val_pred = clf.predict(X_img_val, verbose=0).argmax(axis=1)
fig, axes = plt.subplots(1, 6, figsize=(14, 2.8))
for ax, img, true_i, pred_i in zip(axes, X_img_val[:6],
                                   y_img_val[:6],
                                   val_pred[:6]):
    ax.imshow(img.astype("uint8")); ax.axis("off")
    ok = (true_i == pred_i)
    ax.set_title(f"{wanted[pred_i]}",
                 color="green" if ok else "red",
                 fontsize=10)
plt.suptitle("Fine-tuned model on YOUR categories")
plt.tight_layout()
plt.show();
```
...
```{python}
#| eval: false
# Equivalent pattern in raw PyTorch (run in an environment
# with torchvision):
...
# new head
net.classifier[1] = nn.Linear(net.last_channel, len(wanted))

# the model's own preprocessing
prep = weights.transforms()
# only the head has gradients
opt = torch.optim.Adam(net.classifier.parameters(), lr=1e-3)
# expects raw logits + integer labels
loss_fn = nn.CrossEntropyLoss()

# Reuse the same images/labels; torchvision wants channels-
# first tensors in [0, 1]
# NHWC -> NCHW
X_img_train_t = torch.tensor(X_img_train).permute(0, 3, 1, 2) / 255.0
X_img_val_t = torch.tensor(X_img_val).permute(0, 3, 1, 2) / 255.0
y_img_train_t = torch.tensor(y_img_train)
y_img_val_t = torch.tensor(y_img_val)

train_loader = DataLoader(
    TensorDataset(X_img_train_t, y_img_train_t),
    batch_size=16, shuffle=True)

# Same training loop shape as Modules 1 and 3
for epoch in range(5):
    net.train()
    for x_batch, y_batch in train_loader:
        # clear last step's gradients
        opt.zero_grad()
        # prep applies the model's normalization
        loss = loss_fn(net(prep(x_batch)), y_batch)
        # measure how to improve
        loss.backward()
        # nudge the head's weights
        opt.step()

# Evaluate the same way as the Keras model above
net.eval()
with torch.no_grad():
    logits = net(prep(X_img_val_t))
    val_acc = (logits.argmax(1) == y_img_val_t).float().mean().item()
print("PyTorch validation accuracy:", round(val_acc, 3))
```
...
```{python}
# Scratch space for 2.2: paste clf.summary() output here,
# or try a suggested tweak.
```
...
stream = load_dataset("fancyzhx/yelp_polarity",
...
```{mermaid}
%%| fig-cap: "From messy text to a sentiment call: tokenize, pad, embed, pool, predict."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  R[/"Raw review:<br/>'great food,<br/>slow service'"/]:::data --> T["Tokens to<br/>integer IDs"]:::proc
  T --> PD["Pad to a<br/>fixed length"]:::proc
  PD --> EM["Embedding:<br/>meaning as numbers"]:::accent
  EM --> AV["Average the<br/>word vectors"]:::proc
  AV --> O[/"Positive /<br/>Negative"/]:::good
  classDef data fill:#d1f0ec,stroke:#0b7a75,color:#1a1a1a
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
...
X_text_train, X_text_val, y_text_train, y_text_val = train_test_split(
    X_text, sentiments, test_size=0.2, random_state=2026,
    stratify=sentiments)
...
```{python}
#| eval: false
# TensorFlow-backed text vectorization. Colab ships
# TensorFlow, so this runs as-is.
from keras.layers import TextVectorization

vectorizer = TextVectorization(
    max_tokens=VOCAB_SIZE, output_sequence_length=MAX_LEN)
# learns the vocabulary from the reviews (via TF)
vectorizer.adapt(texts)
demo = vectorizer(["the staff were friendly and the food arrived hot"])
# (1, MAX_LEN)
print("TextVectorization output shape:", tuple(demo.shape))
...
text_model.compile(optimizer="adam",
                   ...
                   metrics=["accuracy"])
text_hist = text_model.fit(
    X_text_train, y_text_train,
    validation_data=(X_text_val, y_text_val), epochs=5,
    batch_size=64, verbose=0)
print(
    "Validation accuracy:",
    round(float(text_hist.history["val_accuracy"][-1]), 3))
```
...
```{python}
# A bar chart turns raw probabilities into something you can
# show a stakeholder
...
plt.yticks(range(len(examples)),
           [s[:32] + "..." for s in examples], fontsize=8)
plt.axvline(0.5, color="k", ls="--", lw=1)
plt.xlabel("predicted positivity"); plt.xlim(0, 1)
plt.title("Sentiment score per review")
plt.tight_layout()
plt.show();
```
...
optimizer = torch.optim.Adam(sentiment_net.parameters(),
                             lr=1e-3)
loss_fn = nn.BCEWithLogitsLoss()
...
```{python}
# Idea 1: more vocabulary and a longer window often help:
# try it
...
X_big_train, X_big_val, y_big_train, y_big_val = train_test_split(
    X_big, sentiments, test_size=0.2, random_state=2026,
    stratify=sentiments)
...
big_hist = big_model.fit(
    X_big_train, y_big_train,
    validation_data=(X_big_val, y_big_val), epochs=5,
    batch_size=64, verbose=0)
print("Bigger-vocab validation accuracy:",
      round(float(big_hist.history["val_accuracy"][-1]), 3))
```

```{mermaid}
%%| fig-cap: "Averaging word vectors throws away order, so the 'not' no longer flips the word after it."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  N[/"word:<br/>'not'"/]:::proc --> AV["Averaged together<br/>(word order is lost)"]:::accent
  G[/"word:<br/>'great'"/]:::proc --> AV
  AV --> R[/"One blended vector<br/>the 'not' can't flip it"/]:::bad
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef bad fill:#fde0e0,stroke:#c62828,color:#1a1a1a
```
...
```{python}
# Stress-test: feed deliberately tricky reviews (sarcasm,
# negation) and watch where a word-averaging model slips:
# it has no sense of word ORDER, so "not great" looks
# positive (it saw "great"). Add your own lines and see
# which ones fool it.
tricky = [
...
```{python}
# Scratch space for 3.2: assemble the numbers/examples
# you'll paste into your assistant.
```
...
```{mermaid}
%%| fig-cap: "The confusion matrix in business terms. The missed cancellation is the costly box."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '15px'}}}%%
flowchart LR
  A[/"Actually<br/>CANCELED"/]:::anchor --> TP["Caught it<br/>(correct)"]:::good
  A --> FN["MISSED it<br/>unsold room (costly)"]:::bad
  K[/"Actually<br/>KEPT"/]:::anchor --> TN["Correct keep"]:::good
  K --> FP["False alarm<br/>(hassled a guest)"]:::warn
  classDef anchor fill:#eceff1,stroke:#546e7a,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
  classDef bad fill:#fde0e0,stroke:#c62828,stroke-width:2px,color:#1a1a1a
...
# The bar to beat: always guessing the majority class ("not
# canceled")
...
plt.title("Where the model is right, and wrong")
plt.show()
...
```{mermaid}
%%| fig-cap: "The decision threshold is a business dial: trade recall against precision by cost."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart TD
  T{"Decision threshold<br/>YOU set it, not 0.5"}:::decision -->|move lower| MORE["Catch more cancels<br/>more recall, less precision"]:::accent
  T -->|move higher| FEWER["Fewer false alarms<br/>more precision, less recall"]:::proc
  MORE --> COST["Set by cost:<br/>a miss costs about<br/>4x a false alarm"]:::bad
  FEWER --> COST
  classDef decision fill:#fff3cd,stroke:#d39e00,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
...
```{python}
# The trade-off, drawn: as the line moves left, you catch
# more but cry wolf more
...
plt.axvline(0.35, color="k", ls="--", lw=1,
            label="a recall-leaning choice")
...
```{python}
# Suppose missing a cancellation costs 4x a false alarm ->
# favor recall
chosen = 0.35
...
```{python}
# Permutation importance: shuffle each feature in turn and
# measure the accuracy it costs. Bigger drop = the model
# relied on that feature more. (Reuses the Module 1 model +
# X_val.)
import numpy as np
rng = np.random.default_rng(2026)

base_acc = ((model.predict(X_val, verbose=0).ravel() > 0.5).astype(int) == y_val).mean()
drops = []
for j in range(len(features)):
    X_shuffled = X_val.copy()
    # break this one feature's signal
    X_shuffled[:, j] = rng.permutation(X_shuffled[:, j])
    acc = ((model.predict(X_shuffled, verbose=0).ravel() > 0.5).astype(int) == y_val).mean()
    drops.append(base_acc - acc)

order = np.argsort(drops)
plt.barh([features[i] for i in order],
         [drops[i] for i in order], color="steelblue")
plt.xlabel("accuracy lost when the feature is shuffled")
plt.title("What drives the cancellation model?")
plt.tight_layout()
plt.show();
```
...
```{python}
# Scratch space for 4.2: paste your threshold's
# precision/recall and the top features here.
```
...
```{mermaid}
%%| fig-cap: "Export once to a SavedModel; standard serving tools load it with no training code."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}}}%%
flowchart LR
  TR(["Trained Keras model<br/>(PyTorch backend)"]):::start --> EX["model.export()"]:::key
  EX --> SM[("TensorFlow<br/>SavedModel")]:::data
  SM --> SRV["Serving stack:<br/>TF Serving, Vertex AI"]:::accent
  SRV --> PR[/"Live<br/>predictions"/]:::good
  classDef start fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
  classDef key fill:#fff3cd,stroke:#d39e00,color:#1a1a1a
  classDef data fill:#d1f0ec,stroke:#0b7a75,color:#1a1a1a
  classDef accent fill:#ede0ff,stroke:#6a1b9a,color:#1a1a1a
  classDef good fill:#d8f5d8,stroke:#2e7d32,color:#1a1a1a
```
...
```{python}
#| eval: false
# Export the trained Module 1 cancellation model as a
# TensorFlow SavedModel. `model.export()` writes a TF
# SavedModel regardless of the active Keras engine.
# -> a folder a serving stack can load
model.export("hotel_cancel_savedmodel")

# Reload it the way a deployment/serving environment would,
# then predict.
import tensorflow as tf
reloaded = tf.saved_model.load("hotel_cancel_savedmodel")
serving = reloaded.signatures["serving_default"]
print("Served predictions:",
      serving(tf.constant(X_val[:3], dtype=tf.float32)))
```
