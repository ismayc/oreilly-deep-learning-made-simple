...
```{python}
# We'll build this together. Two choices flip this from a
# classifier to a REGRESSION model -- notice each one as we
# go: * the output Dense layer has NO activation, so it can
# predict ANY dollar amount * the loss is "mse" (mean
# squared error), which measures how many dollars off we are
```
