...
computation underneath. The solutions file also includes an **idiomatic PyTorch**
...
```{python}
# Data prep is the same recipe as the walkthrough: run it
# and move on.
...
# validation (same rule as the walkthrough)
...
```{python}
# STEP 1: Build the model. We'll do this together. Two
# choices flip it from a classifier to a REGRESSION model,
# so code each one:
#   * the output Dense layer has NO activation, so it can
#     predict ANY dollar amount (not squashed to 0-1)
#   * compile with loss "mse" (mean squared error) and
#     metrics ["mae"], so the score reads as dollars off
# Same body as the classifier (Dense 32 -> Dense 16), then a
# Dense(1) output. Name it reg_model, then compile it.
```

```{python}
# STEP 2: Train and report. Fit reg_model on the regression
# split for a few epochs, then print the validation MAE:
# on average, how many dollars off the price predictions are.
```

```{python}
# Visualize results: predicted vs actual price (points on
# the dashed line = perfect)
...
```{python}
# YOUR CALL: pick 3 categories that exist in class_names
# (print class_names to browse). Swap these for whatever
# "product catalog" you want to recognize.

# Map each Food-101 category index to a fresh 0/1/2 label
# for our 3-class problem


# Build a small labeled set (~120 images each): run this
# to assemble your data.
```

```{python}
# The transfer-learning move (we'll build this together):
# keep the base FROZEN so its general vision is preserved,
# and train only the small new head on top.
```

```{python}
# Visualize: 6 validation photos with the model's call
...
for ax, img, true_i, pred_i in zip(axes, X_img_val[:6],
    ...
    ax.set_title(
        wanted[pred_i],
        color="green" if true_i == pred_i else "red",
        fontsize=10)
...
```{python}
# Lever to pull: a BIGGER vocabulary and a LONGER window.
# Run it, then compare the validation accuracy to the
# walkthrough: more capacity doesn't always win.
# <- try other sizes and see
...
```{python}
# YOUR TURN: write tricky reviews (sarcasm, "not great",
# "would not recommend") and see where a word-averaging
# model slips. Add your own lines to this list.
...
```{python}
# The probabilities come from the Module 1 cancellation
# model.
```
...
```{python}
# YOUR DECISION: missing a cancellation costs ~4x a false
# alarm, so lean toward recall. Change `chosen` and read the
# consequence out loud in business terms.
```
