...
print("Regression features:", Xr.shape)
```

```{python}
# We'll build this together. Two choices make this a REGRESSION model, not a
# classifier -- decide them as a class:
#   * the output Dense layer's activation (what lets it predict ANY dollar amount?)
#   * the loss function (which loss measures dollars-off?)
reg_model = keras.Sequential([
    keras.layers.Input(shape=(Xr.shape[1],)),
    keras.layers.Dense(32, activation="relu"),
    keras.layers.Dense(16, activation="relu"),
    keras.layers.Dense(1),                    # <- your call: activation here, or none?
])
reg_model.compile(optimizer="adam", loss="mse", metrics=["mae"])   # <- your call: which loss?
...
VOCAB2, LEN2 = 15000, 200                     # <- try other sizes and see
